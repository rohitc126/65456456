...
namespace BusinessLayer.DAL
{
    public class DAL_SPECIFIC_GRAVITY_TEST
    {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    _list.Add(new Specific_GRavity_Datalist
                    {
                        ...
                        SAMPLE_COUNT = Convert.ToDecimal(row["SAMPLE_COUNT"] == DBNull.Value ? 0 : row["SAMPLE_COUNT"]),
        ...
        public SpGRavitySample2cs VIEW_Specific_Gravity_Entry(decimal Test_ID)
        {

            SqlParameter[] param = { new SqlParameter("@Test_ID", Test_ID) };

            DataSet ds = new DataAccess(sqlConnection.GetConnectionString_SGX()).GetDataSet("[AGG].[USP_INSERT_SPECIFIC_GRAVITY_VIEW]", CommandType.StoredProcedure, param);
            SpGRavitySample2cs _objSpGravity = null;
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    _objSpGravity = new SpGRavitySample2cs();
                    _objSpGravity.Test_ID = Convert.ToInt32(row["Test_Dtl_ID"] == DBNull.Value ? 0 : row["Test_Dtl_ID"]);
                    _objSpGravity.Test_No = Convert.ToString(row["Test_No"]);
                    _objSpGravity.Date = Convert.ToDateTime(row["Date"]);
                    _objSpGravity.Source = Convert.ToString(row["Source"]);
                    _objSpGravity.Type = Convert.ToString(row["Colour"]);
                    _objSpGravity.Colour = Convert.ToString(row["Colour"]);
                    _objSpGravity.Texture = Convert.ToString(row["Texture"]);
                    _objSpGravity.Shape = Convert.ToString(row["Shape"]);
                    _objSpGravity.Rock_Type = Convert.ToString(row["Rock_Type"]);
               
                    _objSpGravity.Dry_Agg_Wt = Convert.ToDecimal(row["Dry_Agg_Wt"] == DBNull.Value ? 0 : row["Dry_Agg_Wt"]);
                    _objSpGravity.Oven_Dry_Agg_Wt = Convert.ToDecimal(row["Oven_Dry_Agg_Wt"] == DBNull.Value ? 0 : row["Oven_Dry_Agg_Wt"]);
                    _objSpGravity.Sample_Wt = Convert.ToDecimal(row["Sample_Wt"] == DBNull.Value ? 0 : row["Sample_Wt"]);
                    _objSpGravity.Wb_Agg_Wt = Convert.ToDecimal(row["Wb_Agg_Wt"] == DBNull.Value ? 0 : row["Wb_Agg_Wt"]);
                    _objSpGravity.Wb_Wt = Convert.ToDecimal(row["Wb_Wt"] == DBNull.Value ? 0 : row["Wb_Wt"]);
                    _objSpGravity.Dry_Agg_Wt1 = Convert.ToDecimal(row["Dry_Agg_Wt"] == DBNull.Value ? 0 : row["Dry_Agg_Wt"]);
                    _objSpGravity.Oven_Dry_Agg_Wt1 = Convert.ToDecimal(row["Oven_Dry_Agg_Wt"] == DBNull.Value ? 0 : row["Oven_Dry_Agg_Wt"]);
                    _objSpGravity.Sample_Wt1 = Convert.ToDecimal(row["Sample_Wt"] == DBNull.Value ? 0 : row["Sample_Wt"]);
                    _objSpGravity.Wb_Agg_Wt1 = Convert.ToDecimal(row["Wb_Agg_Wt"] == DBNull.Value ? 0 : row["Wb_Agg_Wt"]);
                    _objSpGravity.Wb_Wt1 = Convert.ToDecimal(row["Wb_Wt"] == DBNull.Value ? 0 : row["Wb_Wt"]);
                   
                }

            }

            return _objSpGravity;
        }
    }
}
